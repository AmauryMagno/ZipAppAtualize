...
namespace Atualizate
{
    ...
    public partial class MainWindow : Window
    {
        ...
        {
            UpdateProgress(0, "Atualiate Start");
        ...
        private async void Initialize_Conection()
        {
            try
            {
                ...
                UpdateProgress(10, "Conexão com servidor estabelecida!");
        ...
        private void DeleteAllFilesExcept(string folderPath, string folder1)
        {
            try
            {
                ...
                _log.Log("Devidos arquivos excluidos para atualização");
            }
            catch(Exception ex)
            {
        ...
        private async void GetRemoteVersionAsync()
        {
            try
            {
                ...
                UpdateProgress(15, "Diretórios do arquivo json atual e do novo verificados e criados!");
                ...
                UpdateProgress(20, "Arquivo remoto .json, baixado!");

                //Passo 3: Verifica se há atualização
                if (NeedsUpdate(currentJson, newJson))
                {
                    UpdateProgress(30, "Atualização encontrada");
                    ...
                    UpdateProgress(40, "Atualização, baixada");

                    //Passo 4: Processar atualização
                    ProcessUpdate(versionPath, currentDirectory);
            ...
            {
                _log.Log($"ERRO: Nenhuma atualização encontrada {err.Message}");
                Application.Current.Shutdown();
            }
            catch (Exception ex)
            {
        ...
        private async void UpdateProgress(int progress, string message)
        {
        ...
        private async void ProcessUpdate(string versionPath, string currentDirectory)
        {
            ...
            //camScanProcess.Kill();
            UpdateProgress(50, "Aplicação principal encerrada");

            DeleteAllFilesExcept(currentPath, atualizatePath);
            UpdateProgress(55, "Arquivos da pasta principal deletados");

            UpdateProgress(60, "Descompactando atualização...");
            ZipFile.ExtractToDirectory(zipPath, currentPath);
            UpdateProgress(65, $"Descompactação concluida!");

            DeleteOutdatedFiles(Path.Combine(versionPath, "newversion"), zipPath);
            UpdateProgress(70, "Arquivo zip deletado");

            _gitClient.UpdateJsonConfig(Path.Combine(versionPath, "version.json"));
            UpdateProgress(80, "Novo arquivo .json salvo");
        ...
        private void RestartApplication(string currentPath)
        {
            ...
            if (File.Exists(executablePath))
            {
                ...
                UpdateProgress(90, "Aplicação principal reiniciada");
            }
            else
            {
